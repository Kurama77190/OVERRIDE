# level06: REVERSE KEYGEN LOGIN + SERIAL
...
Résultat : la vérification échoue. Le binaire attend une correspondance exacte entre le login et le serial.

---

### Reverse Engineering

L’objectif est de comprendre la relation entre le `login` et le `serial`.  
Une inspection du binaire (en GDB ou via recompilation) montre que le programme utilise une fonction `auth()` pour valider l’utilisateur.
...
<details>
<summary> Voir le code ASM </summary>
...
### Analyse de la fonction auth()

Fonction `auth()` simplifiée :
...
```
*Clique [**ICI**](./Ressources/main.c) pour voir tout le code reverse en **C***

---

### Générateur C de Login + Serial

*Voici un [**générateur**](./Sources/keygen.c) qui crée aléatoirement des logins valides et calcule leur serial :*
...
void generate_login(char *login) {
...
```bash
./keygen
Login : KdYteQ   Serial : 6231851
```

3. Utiliser le couple dans le binaire :
